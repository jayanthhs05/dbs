...
class ContestAdmin(admin.ModelAdmin):
    ...
    fields = ('name', 'description', 'start_time', 'end_time', 'problems', 'participants')
...
class CommentAdmin(admin.ModelAdmin):
    ...
    def target(self, obj):
        if obj.problem: return f"Problem: {obj.problem.title}"
        if obj.submission: return f"Submission: {obj.submission.id}"
        if obj.contest: return f"Contest: {obj.contest.name}"
        return "-"
